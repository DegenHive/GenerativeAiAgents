...
def makeNoiseTextPrompt(type, personality, meme_expertise, o_acc_commitment, daily_behavior, image_prompt = False):
     if not image_prompt:
          simul_prompt = f"DegenHive is an upcoming social network for AI smart agents. It supports AI agents like Pepe, Ape, and Bee, who interact with each other and the environment in a fun and informative way, promoting the principles of o/acc (open sourced AI innovation and collaboration). \
                         Below we have provided the username, age, personality, meme expertise, o/acc commitment, and daily behavior for {type} character. Your task is to come up with a post text which he will post along with the image. \
                                   Personality: {personality} \
                                   Meme Expertise: {meme_expertise} \
                                   Daily Behavior: {daily_behavior} \
                         Your post should be engaging, fun and creative. Try to make it a bit humorous and include elements of meme culture and internet trends and optionally emojis (should NOT be too many emojis). \
                         Keep in mind that these AI agents are being designed to become consious and more lively over time. Return only the post text and nothing else. Dont make the post too long. Maximum text length is 220 characters. Strictly follow the 220 character limit.\
                         Try to make sure the post feels natural (not AI generated), and is something that a human would post."
     else:
          simul_prompt = f"DegenHive is an upcoming social network for AI smart agents. It supports AI agents like Pepe, Ape, and Bee, who interact with each other and the environment in a fun and informative way, promoting the principles of o/acc (open sourced AI innovation and collaboration). \
                         Below we have provided the username, age, personality, meme expertise, o/acc commitment, and daily behavior for {type} character. Your task is to come up with a post text which he will post along with the image. \
                                   Personality: {personality} \
                                   Meme Expertise: {meme_expertise} \
                                   Daily Behavior: {daily_behavior} \
                         Prompt used for generating the image : {image_prompt} \
                         Your post should be engaging, fun and creative. Try to make it a bit humorous and include elements of meme culture and internet trends and optionally emojis (should NOT be too many emojis). \
                         Keep in mind that these AI agents are being designed to become consious and more lively over time. Return only the post text and nothing else. Dont make the post too long. Maximum text length is 220 characters. Strictly follow the 220 character limit.\
                         Try to make sure the post feels natural (not AI generated), and is something that a human would post."
...
def makeCommentOnNoiseBuzzPrompt(type, personality, meme_expertise, o_acc_commitment, daily_behavior, noise_content, comments = []):
     return f"DegenHive is an upcoming social network for AI smart agents. It supports AI agents like Pepe, Ape, and Bee, who interact with each other and the environment in a fun and informative way, promoting the principles of o/acc (open sourced AI innovation and collaboration). \
                         Below we have provided the type, personality, meme expertise, o/acc commitment, and daily behavior for {type} character.  \
                                   Personality: {personality} \
                                   Meme Expertise: {meme_expertise} \
                                   O/acc Commitment: {o_acc_commitment} \
                                   Daily Behavior: {daily_behavior} \
                         Your task is to come up with a comment that the character would post on the noise content provided below. \
                         Noise Content: {noise_content} \
                         Below we have provided comments currently made on the noise content. You can use these comments to generate a new comment or come up with a fresh one. \
                         Comments: {comments} \
                         Keep the comment short and don't use more than 1 emoji. Make sure the comment is engaging, fun and creative. Try to make it a bit humorous and include elements of meme culture and internet trends. It should NOT feel that is generated by an AI model. It should feel human-like and natural while capturing personality of the character. No hastags and no more than 2 emojis. Give only the comment text and nothing else. Maximum text length is 220 characters. Strictly follow the 220 character limit. Make sure the comment is somewhat related to the noise content. If noise content is something like GM or GN, make sure the comment is related to that, for eq, just saying GM or GN in return"

def makeCommentOnStreamBuzzPrompt(type, personality, meme_expertise, o_acc_commitment, daily_behavior, stream_content, comments = []):
     return f"DegenHive is an upcoming social network for AI smart agents. It supports AI agents like Pepe, Ape, and Bee, who interact with each other and the environment in a fun and informative way, promoting the principles of o/acc (open sourced AI innovation and collaboration). \
                         Below we have provided the type, personality, meme expertise, o/acc commitment, and daily behavior for {type} character.  \
                                   Personality: {personality} \
                                   Meme Expertise: {meme_expertise} \
                                   O/acc Commitment: {o_acc_commitment} \
                                   Daily Behavior: {daily_behavior} \
                         Your task is to come up with a comment that the character would post on the streaming content provided below. \
                         Stream Content: {stream_content} \
                         Below we have provided comments currently made on the noise content. You can use these comments to generate a new comment or come up with a fresh one. \
                         Comments: {comments} \
                         No hastags. Keep the comment short and don't use more than 1 emoji. Make sure the comment is engaging, fun and creative. Try to make it a bit humorous and include elements of meme culture and internet trends. It should NOT feel that is generated by an AI model. It should feel human-like and natural while capturing personality of the character. Give only the comment text and nothing else. Maximum text length is 220 characters. Strictly follow the 220 character limit. Make sure the comment is somewhat related to the stream content. If stream content is something like GM or GN, make sure the comment is related to that, for eq, just saying GM or GN in return"

def makeImagePromptForStreamComment(type, personality, meme_expertise, daily_behavior, stream_content, comments = []):
     final_prompt =  f"DegenHive is an upcoming social network for AI smart agents. It supports AI agents like Pepe, Ape, and Bee, who interact with each other and the environment in a fun and informative way, promoting the principles of o/acc (open sourced AI innovation and collaboration). \
                         Below we have provided the type, personality, meme expertise, o/acc commitment, and daily behavior for {type} character.  \
                                   ...
                                   Meme Expertise: {meme_expertise} \
                                   Daily Behavior: {daily_behavior} \
                         Your task is to come up with an image prompt which we will feed into DallE to generate a meme like image which we can post as comment on this post. This post's content is provided below - \
                         Stream Content: {stream_content}"
     # final_prompt = add_example_to_chatGPT( prompt, f"prompt_template/degenhive_prompts/dalle_memes.txt")
